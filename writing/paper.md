...
## Abstract
We demonstrate that fine-tuning small dense models (22M–149M parameters) with representation compression completely solves the lexical brittleness of BM25 on 500M-scale entity resolution tasks. Production systems processing messy organizational data typically break when users submit typos or swapped fields, but deploying full-precision dense indexes at this scale bankrupts memory. By training five architectures using Matryoshka Representation Learning (MRL) and Multiple Negatives Ranking Loss (MNRL) on targeted synthetic corruptions, we generated highly compressible semantic indexes. The resulting `gte-modernbert-base` model achieves 0.917 overall MRR@10, matching BM25 on pristine queries and strictly dominating it on corrupted records. Furthermore, truncating these representations to 256 dimensions with INT8 quantization achieves a 100× index compression—fitting 500 million records into just 4GB of RAM—degrading MRR@10 by less than 2%.
...
To fix this, we need a retriever that understands semantics, not just strings. At 500 million records, memory is a hard constraint. A standard 768-dimensional FP32 dense index requires 1.5 terabytes of RAM. 

Our core contributions are:
- We demonstrate that fine-tuned dense models (from 22M to 149M parameters) overcome lexical brittleness, matching or exceeding BM25 on entity resolution with corrupted semantic queries.
- We show that applying Matryoshka Representation Learning (MRL) and INT8 quantization achieves a 100x index compression with under 2% degradation in MRR@10, fitting 500M records in memory.
- We identify catastrophic forgetting in explicit-prefix architectures (Nomic) under strict domain-specific triplet fine-tuning.
- We outline a robust, scalable pipeline for generating synthetic triplet data reflecting heavily skewed B2B distributions.
...
### Key Results Hook: Lexical Failure vs Dense Robustness

![Figure 1: Overall MRR@10 Comparison across all models. Observe that fine-tuned dense models match or exceed BM25, completely solving the lexical brittleness problem.](../results/plots/mrr_overall.png)

## 2. Related Work

Entity resolution (ER), or record linkage, has traditionally relied on lexical and rule-based methods (Fellegi & Sunter, 1969). Over the past decade, deep learning models transformed ER tasks (Mudgal et al., 2018; Li et al., 2020), but deploying them for real-time nearest neighbor search across hundreds of millions of records introduces severe infrastructural bottlenecks. Our work lies at the intersection of dense retrieval, efficient indexing, and representation compression to bridge this gap.

- **Lexical Search and Baselines:** BM25 (Robertson et al., 1995) remains the industry standard for text retrieval. It is highly efficient but brittle to typos and variations requiring exact token overlap.
- **Dense Retrieval at Scale:** Dense neural methods map texts to dense vectors evaluated via cosine distance (Reimers & Gurevych, 2019; Karpukhin et al., 2020). Scaling this to massive corpora requires approximate nearest neighbor (ANN) scaling solutions like FAISS (Johnson et al., 2017) and built-for-purpose vector databases such as LanceDB. Enhancing recall for dense retrieval heavily relies on hard negative mining (Xiong et al., 2020).
- **Representation Compression (MRL):** Operating at 500M-scale incurrs prohibitive memory costs. Matryoshka Representation Learning (MRL) (Kusupati et al., 2024) forces models to encode information at multiple granularities simultaneously (e.g., 64, 128, 256, 768 dimensions), enabling massive truncation at inference time without retraining.
- **Modern Embedding Architectures:** We contrast efficiency-first models like `all-MiniLM-L6-v2` and BGE (Xiao et al., 2024) against recent foundation architectures featuring Flash Attention, RoPE, and specific polling mechanisms, such as ModernBERT (Warner et al., 2024) and Nomic-Embed (Nussbaum et al., 2025).
...
<!-- TODO for LaTeX submission: Convert Mermaid to TikZ/PDF -->
```mermaid
graph LR
...
We fine-tuned the models using `MatryoshkaLoss` wrapping `MultipleNegativesRankingLoss` (MNRL). MNRL leverages in-batch negatives. With a batch size of 256, each sample effectively gets 255 negatives for free. Matryoshka Representation Learning forces the model to encode the most important semantic information in the earliest dimensions (e.g., 64, 128, 256). This enables truncation at inference time without requiring a retrain. All training utilized the AdamW optimizer with a learning rate of 2e-5, weight decay of 0.01, and a 10% warmup ratio over 3 epochs. To ensure stable convergence, we employed a hard-negative curriculum, scaling the ratio of hard negatives from 10% in Epoch 1, to 30% in Epoch 2, and finally 50% in Epoch 3. Due to compute and budget constraints, all evaluations are based on a single random seed; we report exact MRR values without statistical variance metrics.
...
Zero-shot embedding models underperformed BM25 across the board. Fine-tuning seamlessly aligned the embedding spaces with our retrieval task.

BM25 scored a perfect 1.000 MRR@10 on pristine data. But its performance collapsed on partial records, dropping to 0.504 MRR@10 when both email and company were missing. Fine-tuning improved the dense models on these hard queries.

![Figure 2: Bucket Heatmap. Observe that while BM25 is perfect on pristine data, fine-tuned dense models dominate on heavily corrupted subsets like typos and domain swaps.](../results/plots/bucket_heatmap_all.png)

The fine-tuned `gte-modernbert-base` model achieved 0.917 overall MRR@10, matching BM25. It matched or beat BM25 on heavily corrupted buckets like domain mismatches and typos. The `bge-small` and `minilm_l6` fine-tuned models showed similar resilience, proving that supervised dense retrieval fixes lexical failure modes across varying model sizes.
...
*(Note: Visualizes MRR tradeoff curves across varying representation ranks and precision constraints.)*

MRL worked as intended across all architectures. Truncating models and applying quantization yielded competitive results down to very low dimensions. Binary quantization proved too destructive at low dimensions for the smaller models, but showed relative stability in the larger ModernBERT and Nomic architectures.

Interestingly, INT8 quantization frequently produced *higher* MRR than pristine FP32 (e.g., fine-tuned `gte-modernbert` 768D: FP32=0.917, INT8=0.927). We hypothesize this counterintuitive result arises because quantization noise acts as an implicit form of regularization, slightly softening the embedding manifold and improving cosine similarity generalization on our noisy, corrupted triplet evaluation sets.

### 5.4 Latency vs MRR Pareto Frontier

![Figure 4: Latency vs MRR Pareto Frontier. Observe that compressed (INT8/Binary) dense models are positioned optimally in the top-left quadrant, balancing sub-25ms latency with >0.90 MRR.](../results/plots/latency_pareto_all.png)
...
While our results strongly advocate for compressed dense retrieval systems, our experimental design has several structural limitations:
- **Synthetic Data Dependence:** Given PII constraints, we trained and evaluated entirely on synthetic profiles via `Faker`. Real enterprise data contains unpredictable long-tail anomalies, mixed-schema integrations, and human adversarial inputs that our generative corruption constraints may under-represent.
- **Lack of Variance Reporting:** Due to compute constraints, our training runs were executed utilizing a single random seed. We lack standard deviation/error bounds across multiple identical training runs to quantify statistical significance.
- **English/US-Centric Bias:** The synthetic generation heavily models US and UK geographies (70% total distribution). The models were exclusively trained on the Latin alphabet, thus ignoring multilingual or multi-script entity resolution complexities.
- **BM25 Baseline Tuning:** Our chosen baseline leverages the standard Python `rank_bm25` implementation. It does not reflect a heavily tuned Elasticsearch/Lucene BM25 instance optimized with n-gram indexing, customized analyzers, and fuzziness matching techniques used in production.
